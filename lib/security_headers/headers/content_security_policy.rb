...
module SecurityHeaders
  class ContentSecurityPolicy < BaseParser
    module Rules
      def self.included(base)
        ...
        base.header_rule("Content-Security-Policy") do
          csp_pattern
        end

        base.rule(:csp_pattern) do
        ...
        # CSP Helpers
        #
        base.rule(:csp_value_sequence) { csp_value >> (wsp >> csp_value).repeat(0) }

        base.rule(:csp_directive) { csp_directive_char.repeat }

        base.rule(:csp_value) { csp_value_char.repeat }

        #CSP tokens are any character except space, comma and semicolon
        base.rule(:csp_directive_char) do
          alpha | digit | str("-")
        end

        base.rule(:csp_value_char) do
          match["\x21-\x2b"] |
